...
def create_customer():
    metadata = Metadata(display_in_invoice=True, key="key", value="value")
...
def test_valid_create_customers_request(httpx_mock: HTTPXMock):
    ...
    assert response.billing_configuration.sync_with_provider is True
...
def test_valid_portal_url(httpx_mock: HTTPXMock):
    ...
    assert (
        response
        == "https://app.lago.dev/customer-portal/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaEpJaWt3WkdGbE1qWmxZUzFqWlRnekxUUTJZell0T1dRNFl5MHdabVF4TURabFlqY3dNVElHT2daRlZBPT0iLCJleHAiOiIyMDIzLTAzLTIzVDIzOjAzOjAwLjM2NloiLCJwdXIiOm51bGx9fQ==--7128c6e541adc7b4c14249b1b18509f92e652d17"
    )
...
def test_invalid_portal_url(httpx_mock: HTTPXMock):
    ...
    httpx_mock.add_response(
        method="GET",
        url="https://api.getlago.com/api/v1/customers/invalid_customer/portal_url",
        status_code=404,
        content=b"",
    )
